    ...
    bot.commands.forEach(async command => {
        ...
        if(command.options?.length >= 1) {
            for (let i = 0; i < command.options.length; i++) {
                ...
                if(command.options[i].type === "string") slashcommand[`add${command.options[i].type.slice(0, 1).toUpperCase() + command.options[i].type.slice(1, command.options[i].type.length)}Option`]
                        (option => option
